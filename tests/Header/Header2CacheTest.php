...
class CacheTest extends TestCase
{
    ...
    function testisCached ()
    {
        ...
        $this->assertFalse($c->isCached(time()), 
                'last modified header (yesterday) and param (now)');
        $this->assertTrue($c->isCached(strtotime('last year')), 
                'last modified header (yesterday) and param (last year)');
        unset($c, $_SERVER['HTTP_IF_MODIFIED_SINCE']);
    }
...
//                 'HTTP 200 Ok (simple plain GET)');
//         $r->setHeader('If-Modified-Since', 
//                 $response->getHeader('last-modified'));
//         sleep(3);
//         $response = $r->send();
//         $this->assertEquals(304, $response->getStatus(), 
//                 'HTTP 304 Not Modified (GET with If-Modified-Since set to Last-Modified of prior request');
//         unset($r);
//     }
...
//         $response = $r->send();
//         $this->assertEquals(200, $response->getStatus(), 
//                 'HTTP 200 Ok (POST without If-Modified-Since)');
//         $r->setHeader('If-Modified-Since', $http->date(strtotime('yesterday')));
//         $response = $r->send();
//         $this->assertEquals(200, $response->getStatus(), 
//                 'HTTP 200 Ok (POST with If-Modified-Since == yesterday)');
//         $r->setHeader('If-Modified-Since', $http->date(time() - 3));
//         $response = $r->send();
//         $this->assertEquals(200, $response->getStatus(), 
//                 'HTTP 200 Ok (POST with If-Modified-Since == now)');
//         $r->setHeader('If-Modified-Since', $http->date($lm));
//         sleep(3);
//         $response = $r->send();
//         $this->assertEquals(200, $response->getStatus(), 
//                 'HTTP 200 Ok (POST with If-Modified-Since == Last-Modified)');
//         $this->assertEquals($http->date(), 
//                 $response->getHeader('last-modified'), 
//                 'POST time() == Last-Modified');
//         unset($r);
//     }
}
