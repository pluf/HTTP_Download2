...
class Download2Test extends PHPUnit_Framework_TestCase
{
    ...
    function testsetContentType ()
    {
        ...
        $this->assertEquals('text/html;charset=iso-8859-1', 
                ...
                '$h->headers["Content-Type"] == "text/html;charset=iso-8859-1"');
        unset($h);
    }
...
//             $this->assertEquals('67890', $response->getBody(), $what);
//             $this->assertTrue(
//                     preg_match('/^bytes 95-\d+/', 
//                             $response->getHeader('content-range')), 
//                     'bytes keyword in Content-Range header');
...
//             $this->assertTrue(
//                     preg_match(
//                             '/^multipart\/byteranges; boundary=HTTP_DOWNLOAD-[a-f0-9.]{23}$/', 
//                             $response->getHeader('content-type')), 
//                     'Content-Type header: multipart/byteranges');
//             $this->assertTrue(
//                     preg_match('/Content-Range: bytes 2-23/', 
//                             $response->getBody()), 
//                     'bytes keyword in Content-Range header');
...
//             $this->assertEquals(206, $response->getStatus(), 
//                     'HTTP 206 Partial Content');
//             $this->assertEquals('bytes 2-51/100', 
//                     $response->getHeader('content-range'), 
//                     'bytes keyword in Content-Range header');
...
//             $response = $r->send();
//             $this->assertEquals(200, $response->getStatus(), 'HTTP 200 Ok');
//             $this->assertEquals('100', $response->getHeader('content-length'), 
//                     'full content');
//             $this->assertEquals($complete, $response->getBody(), $what);
...
//         $this->assertEquals(200, $response->getStatus(), 'HTTP 200 Ok');
//         $this->assertEquals($complete, $response->getBody(), $what);
//         $this->assertFalse($response->getHeader('content-length'), 
//                 'Length unknown');
//         $this->assertFalse($response->getHeader('content-range'), 'No range');
...
//             $response = $r->send();
//             $this->assertEquals(200, $response->getStatus(), 'HTTP 200 Ok');
//             $this->assertTrue($response->getHeader('content-length') > 0, 
//                     'Content-Length > 0');
//             $this->assertTrue(
//                     preg_match('/application\/x-(tar|gzip|bzip2|zip)/', 
//                             $t = $response->getHeader('content-type')), 
//                     'Reasonable Content-Type for ' . $type . ' (actual: ' . $t .
//                              ')');
//         }
//         unset($r);
//     }
