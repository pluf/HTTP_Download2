...
class HTTP2
{
    ...
    public function negotiateLanguage($supported, $default = 'en-US')
    {
        ...
        if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            $match = $this->matchAccept($_SERVER['HTTP_ACCEPT_LANGUAGE'], $supp);
            if (! is_null($match)) {
                return $match;
            }
        }
    ...
    public function negotiateCharset($supported, $default = 'ISO-8859-1')
    {
        ...
        if (isset($_SERVER['HTTP_ACCEPT_CHARSET'])) {
            $match = $this->matchAccept($_SERVER['HTTP_ACCEPT_CHARSET'], $supp);
     ...
     *        
     * @throws HTTP2\Exception When connecting fails
    ...
    public function head($url, $timeout = 10)
    {
        ...
        if (! $fp = @fsockopen($p['host'], $port, $eno, $estr, $timeout)) {
            throw new HTTP2\Exception("Connection error: $estr ($eno)");
        }
     ...
     * @param bool $rfc2616
     *            Wheter to output a hypertext note where we're
...
// ]]>
     ...
     *
     * @param string $url
     *            Absolute or relative URI the redirect should
     *            go to.
     ...
     *            HTTP "Link:" header value (without "Link:"),
            ...
            while ($pos < $len - 1) {
                ...
                while ($pos < $len - 1) {
                    ...
                    if ($pname == 'title*') {
                        ...
                        $link[$pname][$lang] = $this->convertCharset($charset, 'utf-8', urldecode($val));
                    } else if ($pname == 'rel' || $pname == 'rev') {
                        if (! isset($link[$pname])) {
                            $link[$pname] = array();
                        }
                        $link[$pname] = array_merge($link[$pname], array_map('trim', explode(' ', $pval)));
